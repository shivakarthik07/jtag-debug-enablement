...
// -----------------------------------------------------------------------
// DEBUG_STATUS register  (FIX 1: added SHIFT_DR advance)
// -----------------------------------------------------------------------
...
`endif
            ...
            // Shift in TDI so the register advances correctly each cycle
            dbg_status_shift <= TDI;
        end
    end
end

// -----------------------------------------------------------------------
// DEBUG_PC register  (FIX 2: shift in TDI instead of 1'b0)
// -----------------------------------------------------------------------
...
always @(posedge TCK or negedge TRST_N) begin
    if (!TRST_N)
        ...
        if (state_capture_dr && (ir_reg == INSTR_DEBUG_PC))
            ...
            dbg_pc_shift <= {TDI, dbg_pc_shift[31:1]};   // FIX: was {1'b0, ...}
    end
end
...
always @(posedge TCK) begin
    if (state_shift_dr && ir_reg == INSTR_DEBUG_PC)
        $display("PC SHIFT tdo=%b shift=%08x time=%0t",
                 dbg_pc_shift[0], dbg_pc_shift, $time);
end
`endif
...
// -----------------------------------------------------------------------
// TDO multiplexer (combinatorial) — registered on falling TCK edge
// -----------------------------------------------------------------------
reg tdo_mux;
...
always @(*) begin
    ...
    else if (state_shift_dr && (ir_reg == INSTR_DEBUG_STATUS))
...
always @(posedge TCK) begin
    if (state_capture_dr && ir_reg == INSTR_DEBUG_PC)
        $display("PC CAPTURE debug_pc=%08x time=%0t", debug_pc, $time);
end

always @(posedge TCK) begin
    if (state_update_dr && ir_reg == INSTR_DEBUG_CTRL)
        $display("CTRL UPDATE shift=%b time=%0t", dbg_ctrl_shift, $time);
end
...
always @(posedge TCK) begin
    if (state_shift_ir)
        $display("IR SHIFT TDI=%b ir_shift=%b time=%0t", TDI, ir_shift, $time);
end

always @(negedge TCK) begin
    if (state_shift_dr && ir_reg == INSTR_DEBUG_PC)
        $display("NEGEDGE TDO=%b dbg_pc_shift=%08x", TDO, dbg_pc_shift);
end

always @(posedge TCK) begin
    if (state_update_ir)
        $display("IR UPDATE ir_shift=%b ir_reg=%b", ir_shift, ir_reg);
end
`endif
